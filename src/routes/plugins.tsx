...
function Plugins() {
  ...
  const [form, setForm] = useState({
    ...
    name: "",
    version: "1.0.0",
    targetApp: "eztrac" as "eztrac" | "rpt",
    category: "tools",
    publisher: "Flex",
    description: "",
  });
  ...
  async function submitPlugin() {
    ...
    if (!pluginId || !name) {
      toast.error("Plugin ID and Name are required.");
      return;
    }

    setSubmitBusy(true);
    try {
      const res = await fetch(`${API_BASE}/api/v1/partner-marketplace/publish`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          publisher: form.publisher.trim() || "Flex",
          category: form.category.trim() || "tools",
          permissions: [],
          datasets: [],
          kind: "extension",
          enabled: true,
      ...
      try {
        ...
        errorText = "API returned non-JSON response. Ensure dev API runs on 3847.";
      }
      if (!res.ok) {
        throw new Error(data.error || errorText || `Publish failed (${res.status})`);
      }

      toast.success(data.message || `${name} published to marketplace.`);
      setSubmitOpen(false);
      setForm({
        pluginId: "",
        name: "",
      ...
      toast.error(err instanceof Error ? err.message : "Unable to submit plugin.");
    } finally {
      setSubmitBusy(false);
    }
  }
  ...
  return (
    <div className="px-6 lg:px-10 py-8 max-w-[1400px] mx-auto space-y-8">
      <PageHeader
        ...
        actions={
          <Button size="sm" variant="outline" className="gap-2" onClick={() => setSubmitOpen(true)}>
      ...
      <Dialog open={submitOpen} onOpenChange={setSubmitOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Publish a plugin entry so it becomes available in partner marketplace.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-1">
            <div className="grid gap-1.5">
              ...
              <Input
                id="pluginId"
                placeholder="example: eztrac-budget-lens"
                value={form.pluginId}
                onChange={(e) => setForm((s) => ({ ...s, pluginId: e.target.value }))}
              />
            </div>
            <div className="grid gap-1.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="grid gap-1.5">
                <Label htmlFor="pluginPublisher">Publisher</Label>
                <Input
                  id="pluginPublisher"
                  value={form.publisher}
                  onChange={(e) => setForm((s) => ({ ...s, publisher: e.target.value }))}
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-1.5">
                ...
                <select
                  id="targetApp"
                  className="h-9 rounded-md border border-input bg-card text-foreground px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                  value={form.targetApp}
                  onChange={(e) => setForm((s) => ({ ...s, targetApp: e.target.value as "eztrac" | "rpt" }))}
                >
                  <option value="eztrac" className="bg-card text-foreground">EzTrac</option>
                  <option value="rpt" className="bg-card text-foreground">dhub-rpt</option>
                </select>
              </div>
              <div className="grid gap-1.5">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setSubmitOpen(false)} disabled={submitBusy}>Cancel</Button>
            <Button onClick={() => void submitPlugin()} disabled={submitBusy}>
              {submitBusy ? "Submitting..." : "Submit plugin"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
